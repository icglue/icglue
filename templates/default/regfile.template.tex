...
<%  foreach_array reg $register_list {
    %>    \getReg{<%=$rf_name ":" $reg(name) ".table\}\%\n" %><%
    }
%>}%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% register
<%  foreach_array reg $register_list {
        %><%= "\{" $rf_name  ":" $reg(name) ".table\}\{\%\n"%><%
        foreach_array sreg $reg(regs) {
            if {[regexp -nocase {\-} $sreg(name)]} {
                %>    \getReg{<%=$rf_name ":" $reg(name) ":" [string map {":" "_"} $sreg(entrybits)] "\_unused.table\}\%\n" %><%
            } else {
                %>    \getReg{<%=$rf_name ":" $reg(name) ":"  $sreg(name) ".table\}\%\n" %><%
            }
        }
        %><%="\}\%\n"%><%
    }
...
foreach_array reg $register_list {
    ...
    foreach_array sreg $reg(regs) {
        %><%=
        if {[regexp -nocase {\-} $sreg(name)]} {
            %>    {<%=$rf_name ":" $reg(name) ":" [string map {":" "_"} $sreg(entrybits)] "\_unused.table\}\{" %><%
        } else {
            %>    {<%=$rf_name ":" $reg(name) ":"  $sreg(name) ".table\}\{" %><%
        }
