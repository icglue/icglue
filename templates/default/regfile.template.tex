...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% register file
\begin{filecontents}{<%=$rf_name ":register.all"%>}
    \input{<%=$rf_name ":reg.header"%>}
<%  foreach_array reg $register_list {
        %>    \input{<%=$rf_name "." $reg(name) "\}\n"%><%
    }
%>\end{filecontents}

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% register header
\begin{filecontents}{<%=$rf_name ":reg.header" %>}
...
<%
    ...
    foreach_array reg $register_list {
        %>\begin{filecontents}{<%=$rf_name "." $reg(name)%>}<%="\n"%><%
        set i_sreg 0
        foreach_array sreg $reg(regs) {
            ...
            %> \input{<%=$rf_name "." $reg(name) "." $sreg(name) "\}\n"%><%
            incr i_sreg
        }
        %>\end{filecontents}<%="\n\n"%><%
    }
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% subregister header
\begin{filecontents}{<%=$rf_name ":subreg.header" %>}
    Name,Width,Access,Align,Reset,Description
\end{filecontents}
...
<%
   #  "Name" "Width" "Access" "Align" "Reset" "Description"
    foreach_array reg $register_list {
        foreach_array sreg $reg(regs) {
        %>\begin{filecontents}{<%=$rf_name "." $reg(name) "." $sreg(name)%>}<%="\n"%><%
            foreach i_column $num_column {
            # Name Width Bits Function
                ...
                if {$i_column == 3} {
                    set text    [tex_escape $sreg(width)]
                }
                if {$i_column == 4} {
                ...
                if {$i_column == 6} {
                    set text    [tex_escape $sreg(reset)]
                }
                if {$i_column == 7} {
...
<%
    foreach_array reg $register_list {
        %>\begin{filecontents}{<%=$rf_name "." $reg(name)%>:reg.address}<%="\n"%><[format "0x%08x" $reg(address)]><%="\n"%>\end{filecontents}<%="\n"%><%
    }
%>
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
